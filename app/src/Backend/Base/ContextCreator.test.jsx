...
const { Provider, Consumer, withHandler } = ContextCreator(
  BackendHandler,
  handlerName
);
...
const Child = (context) => (
  <span role="child">{context?.toString() || "undefined"}</span>
);
...
test("ContextCreator generated context consumer (using useHandler) fails when rendered without a provider", async () => {
  const { output } = await catchConsoleError(async () => {
    expect(() => render(ConsumerCreator())).toThrowError();
  });
  expect(
    output[1].includes(`Handler must be used within a ${handlerName} Provider`)
  ).toBe(true);
});
