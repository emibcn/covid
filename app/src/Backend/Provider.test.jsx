...
import ChartProvider, { withHandler as withChartHandler } from './Charts/context';

const Child = ({mapsDataHandler, bcnDataHandler, chartsDataHandler, ...props}) => (
...
test('a multiple context consumer fails when rendered without any provider', async () => {
  ...
  const {output} = await catchConsoleError( async () => {
    expect( () => render(<WrappedChild data-value="test-props" />) )
      .toThrowError();
  });
...
test('a multiple context consumer fails when rendered with only one provider', async () => {
  ...
  await Promise.all( Providers
    ...
    .map( async (OneProvider) => {
      const {output} = await catchConsoleError( async () => {
        expect( () => render(<OneProvider><WrappedChild data-value="test-props" /></OneProvider>) )
          .toThrowError();
      });
...
test('a multiple context consumer fails when rendered with only two providers', async () => {
  ...
  const ProviderReducer = (Providers) =>
    ({children}) =>
      Providers.reduce(
    ...
    .map( async (OneProvider) => {
      const {output} = await catchConsoleError( async () => {
        ...
        const Selected = Providers
          .filter( pr => pr.name !== OneProvider.name)
          .map(({Provider}) => Provider);

        // Combine selected providers into one component
        const TwoProviders = ProviderReducer(Selected);
        expect( () => render(<TwoProviders><WrappedChild data-value="test-props" /></TwoProviders>) )
          .toThrowError();
      });
 
      expect(output[1].includes(`Handler must be used within a ${OneProvider.name}DataHandler`)).toBe(true);
    })
  );
});

test('a multiple context consumer is rendered properly when rendered inside the multi-provider', async () => {
  const rendered = render(<Provider><WrappedChild data-value="test-props" /></Provider>);
