...
class IndexesHandler extends React.Component {
  ...
  onBeforeUpdate = (backend) => {
    console.log(`onBeforeUpdate: Looking for an update of backend ${backend.handler.name}`);
  }
  ...
  onAfterUpdate = (backend, updated) => {
    console.log(`onAfterUpdate: The backend ${backend.handler.name} was${updated ? '' : ' not'} updated`);
  }
  ...
  updateIfNeeded = (backend) => {
    return new Promise( resolve => {
      ...
      const now = new Date();
      if( this.state[backend.state] !== null &&
  ...
  updateAllIfNeeded = () => Promise.all(
    this.backends.map( this.updateIfNeeded )
  )
