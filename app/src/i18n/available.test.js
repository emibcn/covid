...
test('languages are defined correctly (minimally)', () => {
  ...
  for (const locale of Object.keys(groups)) {
    for (const keyIndex of groups[locale]) {
      expect(available.find(({key}) => key === keyIndex).value.locale).toBe(locale);
    }
  }
});
...
test('all languages define all and only all translations defined in English', () => {
  ...
  const components = Object.keys(english)
    .filter( key => !(['locale', 'name'].includes(key)));
  ...
  for (const component of components) {
    for (const text of Object.keys(english[component])) {
      ...
      try {
        ...
        throw new Error(`Component '${component}' defined in English is NOT defined in language '${language.locale}'`);
      }
      ...
      for (const language of languages) {
        try {
          ...
          throw new Error(`Text key '${text}' defined in component '${component}' defined in English is NOT defined in language '${language.locale}'`);
        }
      }
    }
  }
  ...
  for (const language of languages) {
    const components = Object.keys(language)
      .filter( key => !(['locale', 'name'].includes(key)));
    ...
    for (const component of components) {
      ...
      try {
        ...
        throw new Error(`Component '${component}' defined in language '${language.locale}' is NOT defined in English`);
      }
      ...
      for (const text of Object.keys(language[component])) {
        try {
          ...
          throw new Error(`Text key '${text}' defined in component '${component}' defined in language '${language.locale}' is NOT defined in English`);
        }
      }
    }
  }
